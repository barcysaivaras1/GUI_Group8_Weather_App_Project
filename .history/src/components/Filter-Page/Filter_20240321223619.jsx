...
if (finalArray.length == 0){
        alert("Loading ..."); //gives an alert if final array is empty
	}
	else{
		navigate("/filter/results", { state: { finalArray } }); //carries over finalArray onto results component
	}
}
console.log(values);
console.log('redirected')

const handleButtonPress = (e) => { //changes some styling when buttons are clicked and applies changes in states to the filterArrays
...
const weatherData = useNewWeatherData(capitalCities); // storing weatherData of capitalCities array
	...
	useEffect(() => {
		const filterCitiesByTemperatureAndWeatherType = () => {   // IMPORTANT: finally checks if filteredTempCities include the selected weatherTypes and creates a final Array
			const finalFilter = [];
...
const FilterCountriesByContinent = async (continent) => {                  // filters countries by the continent selected
    ...
    for (const country of countries) {
        ...
        const countryResponse = await fetch(`https://api.countrystatecity.in/v1/countries/${country}`, {     
            headers: {
                'X-CSCAPI-KEY': 'Wmk1Vlhxa3hIVTNBeE9rS3NubE9QSUdheHQ2YmtrQVRPdk1UMERvWg==',
            },
        });
...
const ButtonWeather = ({ handleButtonPress }) => {   //for performing actions on weatherType buttons
return (
...
const ButtonContinent = ({ handleContinentClick }) => {  //for performing actions on continent buttons
return (
