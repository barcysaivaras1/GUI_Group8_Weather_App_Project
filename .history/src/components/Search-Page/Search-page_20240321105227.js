...
function NewWeatherSearchPage() {
    ...
    const [searchTermsArray, setSearchTermsArray] = useState([]); // State to hold the array of search term
    const [cityArrayArray, setCityArray] = useState([]);s
    ...
    const handlecityArrayUpdate = () => {
        setSearchTermsArray(prevcityArray => [...prevcityArray, cityArray]);
        setCityArray('');
        setTyping(false); // User stopped typing
    };
